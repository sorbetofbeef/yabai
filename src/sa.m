...
{
    char* response = mach_send_message(mach_get_bs_port(), "handshake", 9, true);
...
bool scripting_addition_add_to_window_ordering_group(uint32_t child_wid, uint32_t parent_wid)
{
    char message[MAXLEN];
    snprintf(message, sizeof(message), "window_ordering_group_add %d %d", parent_wid, child_wid);
    return scripting_addition_run_command(message);
}

bool scripting_addition_add_to_window_movement_group(uint32_t child_wid, uint32_t parent_wid)
{
    char message[MAXLEN];
    snprintf(message, sizeof(message), "window_movement_group_add %d %d", parent_wid, child_wid);
    return scripting_addition_run_command(message);
}

bool scripting_addition_remove_from_window_ordering_group(uint32_t child_wid, uint32_t parent_wid)
{
    char message[MAXLEN];
    snprintf(message, sizeof(message), "window_ordering_group_remove %d %d", parent_wid, child_wid);
    return scripting_addition_run_command(message);
}

bool scripting_addition_remove_from_window_movement_group(uint32_t child_wid, uint32_t parent_wid)
{
    char message[MAXLEN];
    snprintf(message, sizeof(message), "window_movement_group_remove %d %d", parent_wid, child_wid);
    return scripting_addition_run_command(message);
}
